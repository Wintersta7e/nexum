...
Mirrors the real Codex on-disk layout used by the Codex adapter.

## Files

- `build_state.sql` — SQL source of truth for the SQLite DB. Schema mirrors
  the column projection the adapter SELECTs from `threads` (id /
  rollout_path / cwd / git_origin_url / created_at / updated_at / title /
  git_sha / git_branch / model). NO `sessions` table — real Codex doesn't
  have one; transcripts live in JSONL files.
...
- `nexum-core::project::resolve` integration tests.
